...
def log_triage_job(ctx: JobContext, triage_run_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        artifact_sources = [normalize_artifact_source(item, case) for item in active_artifacts]
        if not provider.is_mock and not case.model_egress_approved:
            plan = log_triage_planning.deterministic_plan(
                case,
                methods,
                patterns,
                reason=(
                    "Case did not approve model egress; used the local deterministic "
                    "planner without calling the configured Chat model."
                ),
            )
            model_result = {
                ...
                "duration_ms": 0,
                "fallback": True,
                "reason": "model_egress_not_approved",
            }
        else:
