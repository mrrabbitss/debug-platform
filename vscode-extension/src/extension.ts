...
function workspaceConfig() {
  ...
  return {
    backend: (c.get<string>('backendUrl') || 'http://127.0.0.1:8765/api/v1').replace(/\/$/, ''),
    web: (c.get<string>('webUrl') || 'http://127.0.0.1:8765/ui').replace(/\/$/, ''),
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Associating current workspace', cancellable: false }, async () => {
      ...
      try {
        ...
        const attached = await axios.post(
          `${c.backend}/cases/${caseId}/workspaces/attach`,
          { path: root, name: path.basename(root) },
          { headers: c.headers, timeout: 30000 },
        )
        const indexed = await axios.post(
          `${c.backend}/workspaces/${attached.data.id}/index`,
          {},
          { headers: c.headers, timeout: 30000 },
        )
