...
onMounted(async () => { try { await loadConfig(); ready.value = true; syncTab(); if (tab.value==='review') await loadLegacyCases() } catch(cause) { error.value = failure(cause) } })
</script>
<template>
  <div class="workspace-page">
    ...
    <template v-if="ready && canManageKnowledge">
      ...
      <SkillManagementPane v-if="tab==='skills'" :categories="config.categories" :roles="config.knowledge_roles" :owner-id="config.principal.id || ''" @changed="refreshConfig" @organize="tab='assistant'" @maintain="tab='maintenance'" />
