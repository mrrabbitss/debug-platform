...
async function submit() {
  ...
  try {
    await api.post('/workbench/library', {
    ...
    ElMessage.success('已提交管理员审核，可在知识库的已定位案例中查看状态')
  } catch (cause) { error.value = failure(cause) }
...
<template>
  <el-dialog :model-value="modelValue" @update:model-value="value => !busy && emit('update:modelValue', value)"
    ...
    <p class="muted">填写已确认的根因、证据和验证结果，管理员审核通过后进入共享知识库。</p>
    ...
    <el-form label-position="top" @submit.prevent="submit">
      ...
      <el-form-item label="导入 Markdown 或文本"><input type="file" accept=".md,.markdown,.txt" aria-label="导入案例文本" :disabled="busy" @change="importText" /></el-form-item>
    ...
    <template #footer><el-button :disabled="busy" @click="emit('update:modelValue', false)">取消</el-button><el-button type="primary" :loading="busy" @click="submit">提交管理员审核</el-button></template>
  </el-dialog>
</template>
