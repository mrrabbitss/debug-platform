...
const { config, isAdmin, canManageKnowledge, roleLabel, configLoading, configError, loadConfig } = useWorkbench()
const activeNavigation = computed(() => route.path.startsWith('/cases') ? '/cases' : route.meta.requiresKnowledgeManager ? '/knowledge-management' : /^\/(knowledge|admin\/curation)/.test(route.path) ? '/knowledge' : '/settings')
  ...
  <el-container class="app-shell">
    <el-aside width="216px" class="sidebar">
      ...
      <nav aria-label="主导航"><el-menu router :default-active="activeNavigation" class="nav-menu">
        ...
        <el-menu-item v-if="canManageKnowledge" index="/knowledge-management"><span class="nav-symbol" aria-hidden="true">◇</span>知识库管理</el-menu-item>
    ...
    <el-container class="workspace-container">
      <el-header class="topbar"><div class="breadcrumb">工作台 <span>/</span> <strong>{{ route.meta.title }}</strong></div>
        <div class="topbar-actions"><el-tag size="small" effect="plain">{{ roleLabel }}</el-tag><span class="identity-label">{{ config.principal.display_name || config.principal.username || '工作台' }}</span></div>
      </el-header>
      <el-main id="workbench-content" class="main-content" tabindex="-1">
        ...
        <el-result v-else-if="route.meta.requiresKnowledgeManager && !canManageKnowledge" icon="warning" title="此页面仅限专家和管理员"><template #extra><el-button @click="$router.replace('/knowledge')">返回知识库</el-button></template></el-result>
