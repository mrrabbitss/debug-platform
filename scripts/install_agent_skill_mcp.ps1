...
function Test-ClientEntry {
    ...
    try {
        # Windows PowerShell 5.1 wraps native stderr as an error record, even
        # for the expected "entry not found" probe during first installation.
...
function Set-CodexToolTimeout {
    # Only edit the server table just generated by `codex mcp add`.
    if ($DryRun) {
        Write-Output "DRY RUN: set mcp_servers.gw-ap-debug.tool_timeout_sec = 7200"
        return
    }
    $configRoot = [Environment]::GetEnvironmentVariable('CODEX_HOME', 'Process')
    if ([string]::IsNullOrWhiteSpace($configRoot)) {
        $configRoot = Join-Path $env:USERPROFILE '.codex'
    }
    $configPath = Join-Path $configRoot 'config.toml'
    $text = [IO.File]::ReadAllText($configPath)
    $pattern = '(?m)^\[mcp_servers\.(?:gw-ap-debug|"gw-ap-debug"|''gw-ap-debug'')\][ \t]*(?=\r?$)'
    $matches = [regex]::Matches($text, $pattern)
    if ($matches.Count -ne 1) {
        throw 'Cannot locate the new Codex MCP table; tool timeout was not changed.'
    }
    $table = $matches[0]
    $tableEnd = [regex]::Match($text.Substring($table.Index + $table.Length), '(?m)^\[')
    $end = if ($tableEnd.Success) { $table.Index + $table.Length + $tableEnd.Index } else { $text.Length }
    $body = $text.Substring($table.Index + $table.Length, $end - $table.Index - $table.Length)
    if ($body -match '(?m)^\s*tool_timeout_sec\s*=') {
        throw 'The new Codex MCP table already has a timeout; review it before changing.'
    }
    $newline = if ($text.Contains("`r`n")) { "`r`n" } else { "`n" }
    $updated = $text.Insert($table.Index + $table.Length, $newline + 'tool_timeout_sec = 7200')
    $temporary = "$configPath.gwap-timeout.tmp"
    try {
        [IO.File]::WriteAllText($temporary, $updated, [Text.UTF8Encoding]::new($false))
        [IO.File]::Replace($temporary, $configPath, "$configPath.before-gwap-timeout.bak")
    } finally {
