...
| LLM 日志规划与三层证据 | `AVAILABLE` | 在案例明确授权后，模型完整阅读 GW/AP/通用联合诊断方法并规划关键词；兼容 GLM JSON 根对象包裹/字面量关键词形态，校验失败时有界纠正一次并保留累计 Token；本地扫描完整提取文本，按“LLM 相关 / 方法必查 / 其他结构化事件”分页展示，保留文件、行号、来源设备/角色、规则与排序理由 |
...
| 证据约束 LLM 诊断 | `AVAILABLE` | 至少两轮、最多八轮规划；GW 主设备与 AP 从设备作为联合诊断域，逐轮阅读两侧全部已发布方法、输出逐方法相关性并为相关方法绑定检查与检索；兼容 GLM-5.1/5.2 富对象 JSON，结构或覆盖校验失败时纠正一次；均衡考虑两侧日志、执行有界检索并记录停止原因；最终结论只允许引用已提供 evidence ID，非法输出回退到确定性结果 |
| 模型网关 | `AVAILABLE` | 前端管理并切换 Chat、Embedding、Reranker 配置；Chat API 支持逐 Profile 加密代理，空值直连，代理启用时保留证书链/主机名校验并跳过吊销检查；GLM-5.1/5.2 可配置 `thinking.type=enabled` 与 `max_tokens`，新 Profile 默认 300 秒超时以容纳长方法规划 |
...
- [x] 综合诊断至少两轮、最多八轮 LLM Planning，每轮重新提供全部联合方法并验证方法覆盖；
...
- [x] Alembic `0014` 保存规划、三层证据索引、问答任务关联和案例授权；`0015` 保存日志设备来源和可审核诊断修订；旧库启动时自动升级；
