...
| LLM 日志规划与三层证据 | `AVAILABLE` | 在案例明确授权后，模型完整阅读全部适用方法并规划关键词；本地扫描完整提取文本，按“LLM 相关 / 方法必查 / 其他结构化事件”分页展示，保留文件、行号、规则与排序理由 |
...
| 证据约束 LLM 诊断 | `AVAILABLE` | 至少两轮、最多三轮规划，逐轮阅读全部适用故障树/分析方法、执行有界检索并记录停止原因；最终结论只允许引用已提供 evidence ID，非法输出回退到确定性结果 |
| 模型网关 | `AVAILABLE` | 前端管理并切换 Chat、Embedding、Reranker 配置；Chat API 支持逐 Profile 加密代理，空值直连，代理启用时保留证书链/主机名校验并跳过吊销检查；GLM-5.1/5.2 可配置 `thinking.type=enabled` 与 `max_tokens` |
...
| 案例对话 | `AVAILABLE` | 基于当前案例、最新诊断和检索证据异步回答；提交立即返回持久化 Job，前端可恢复轮询、取消、查看失败原因和本轮轨迹，避免长模型请求占住 HTTP 连接 |
...
| Agent 运行轨迹 | `AVAILABLE` | 运行中增量记录摘要哈希、方法证据 ID、规划轮次、模型/Prompt、tokens、耗时、重试、停止和审批；案例页可实时查看，管理员可安全只读重放，正文不写入轨迹 |
...
- [x] 日志规划要求模型精确回报全部已读文档 ID，只接受已编译 Pattern ID 和字面量追加关键词；
- [x] 完整提取文本在本机逐行扫描，命中没有结构化事件的命令输出也保留文件与原始行号；
- [x] 模型相关、方法必查、其他结构化事件三层分页展示，重复日志聚类并保留出现次数；
- [x] 综合诊断至少两轮、最多三轮 LLM Planning，每轮重新提供全部适用方法并验证方法覆盖；
- [x] 规划检索查询有界、去重；模型失败或 Schema/证据校验失败时保留确定性基线并记录停止原因；
- [x] 规划轮次、方法读取、检索、最终合成在案例页增量显示，轨迹只保存哈希和安全元数据；
- [x] 案例问答改为可恢复后台 Job，支持状态轮询、取消、失败信息和本轮轨迹，不再依赖长 HTTP 请求；
- [x] GLM-5.1/5.2 Profile 可在前端开启 thinking，并配置长输出 `max_tokens`；同一多轮任务复用单一事件循环，避免异步 HTTP 客户端跨事件循环导致第二轮连接失败；
- [x] Alembic `0014` 保存规划、三层证据索引、问答任务关联和案例授权；旧库启动时自动升级；
...
不存在时系统仍使用知识库中 `ACTIVE` 且适用当前设备的诊断文档。两文件被 Git 忽略，正文不会
