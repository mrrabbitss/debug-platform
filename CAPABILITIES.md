...
0.5.3 安装修复准备交付：已复现并修复旧文件占用导致 PowerShell 逐文件移动、旧程序目录不完整的问题；
发布与回滚改用同父目录重命名，旧目录清理失败不撤回新版。新增三项 WinPS 5.1 故障检查通过。
服务器安装错误增加底层原因与持久日志；最终验证/发布状态见 [安装故障记录](docs/installer-upgrade-fix-20260910.md)
