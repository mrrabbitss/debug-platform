...
| LLM 日志规划与三层证据 | `AVAILABLE` | 在案例明确授权后，模型完整阅读 GW/AP/通用联合诊断方法并规划关键词；本地扫描完整提取文本，按“LLM 相关 / 方法必查 / 其他结构化事件”分页展示，保留文件、行号、来源设备/角色、规则与排序理由 |
...
| 证据约束 LLM 诊断 | `AVAILABLE` | 至少两轮、最多八轮规划；GW 主设备与 AP 从设备作为联合诊断域，逐轮阅读两侧全部已发布方法、均衡考虑两侧日志、执行有界检索并记录停止原因；最终结论只允许引用已提供 evidence ID，非法输出回退到确定性结果 |
