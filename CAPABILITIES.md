...
### 三栏工作台改版（0.4.0，本地实现与定向检查完成）

当前增量以[实施与接续记录](docs/workbench-iteration-20260908.md)及各模块交接为准；[完整服务器EXE与分机ZIP](deploy/releases/20260909/0.4.0/README.md)已构建并通过完整性和隔离包内运行检查。
新规则正式替代旧版个人修订权限：普通用户只读已发布知识、提交案例与报告，管理员确认共享；知识修改/发布仅管理员。
