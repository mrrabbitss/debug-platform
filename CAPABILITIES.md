...
0.5.6 安装修复已通过开发机实际 EXE 验收：服务器改用独立版本目录，不再重命名旧应用或新 staging；
安装准备阶段验证失败明确停止，覆盖安装自动备份并更新指定六文件，首次启动空库自动导入。
停服保护、同时占用新旧目录的覆盖安装、全新安装及首次启动六文件入库均通过；公司机器尚未验收。
变更来源和用户需求区分见 [安装变更审计](docs/installer-056-change-audit.md)，证据以 VALIDATION.md 为准。
已发布 [0.5.6 最新正式版](https://github.com/mrrabbitss/debug-platform/releases/tag/v0.5.6)，
八个远端附件大小及 SHA-256 与本地一致；分机继续沿用字节不变的 0.5.0 包。

新增组网 Skill 独立更新工具：停服后复用现有运行环境，更新指定六文件包并保留其他知识、案例和报告，
不替换程序目录；先备份，完整索引成功后原子发布，持久审批支持中断重试。
入口与边界见 [独立更新使用说明](docs/offline-network-skill-update-20260910.md)，验证见 VALIDATION.md。
此工具是安装故障期间的独立恢复途径；现有 0.5.5 安装器仍未在公司服务器成功验收。
...
此前发布 [0.5.5 正式版](https://github.com/mrrabbitss/debug-platform/releases/tag/v0.5.5)；
...
验证/交付详情见 [安装故障记录](docs/installer-upgrade-fix-20260910.md)
和 [VALIDATION.md](VALIDATION.md)。业务数据和 API/MCP 契约不变，用户服务器的实际触发原因尚未通过日志确认。

本轮核对并补齐常用 Chat 表单的慢模型超时展示，以及候选补丁入口的个人模型/授权快照。
网页 Chat 工作统一显示持久化阶段、实际阅读/核验计数、模型等待、估算剩余比例；
审核修正、提炼对话、模型测试与候选补丁新增可查询后台任务。实现和既有需求映射见
[需求核对记录](docs/requirements-audit-20260910.md)；最新验证与交付状态见 [VALIDATION.md](VALIDATION.md)。
已交付 [0.5.2 正式版](https://github.com/mrrabbitss/debug-platform/releases/tag/v0.5.2)：20 个新增后端场景、
前端进度/时间检查、3 条新浏览器场景及工程检查 24/24 通过；10,217 项安装清单、284 个应用文件与
六文件原包哈希核对通过。7 份远端附件及标签已核验；分机仍沿用相同的 0.5.0 ZIP。

0.5.1 安装修复：补齐指定六文件组网包的本地安装交付、空知识库初始化、管理页面状态及本机管理员恢复。
初始化复用原子发布队列，保留人工修改和删除；旧令牌不自动延期。实现与验证边界见
[安装修复记录](docs/installer-skill-admin-fix-20260909.md)。0.5.0 本身未内置该组网包，之前本机导入不能代表安装交付完成。
0.5.1 已发布为[正式版](https://github.com/mrrabbitss/debug-platform/releases/tag/v0.5.1)，七份附件与源码标签远端核验通过；18 项新增检查及包内新安装/管理员恢复验证通过，实际公司机器的登录失败原因仍待报错信息确认。

0.5.0 慢模型等待优化：Chat 单次默认 15 分钟，旧默认配置同步生效；新建问答、日志规划、
知识提炼/归类及整理任务上限 2 小时。新增 7 项配置/SDK 传递检查通过，详见
[本轮等待与交付记录](docs/slow-model-timeouts-20260909.md)。真实模型已通过项目沿用原证据。0.5.0 已发布，7 项 GitHub 附件及源码标签核验通过。

本日追加了用户授权的真实第三方 Chat API 与 Codex CLI 验证，见
[真实模型验证记录](docs/expert-live-model-validation-20260909.md)。测试发现并修复首次知识
发布误判租约丢失、审核 AI 输出缺少明确结构约束，以及普通用户查询日志规划任务状态的
案例权限关联遗漏；不改变原有审核与模型所有权规则，修复已纳入 0.5.0 完整安装包。

### 专家角色与知识协作（2026-09-09，源码及本机集成完成）

以下权限和模型规则替代后文 0.4.0 / M0–M5 历史规则；已构建 0.5.0 服务器与配套分机，
[发行附件及校验](https://github.com/mrrabbitss/debug-platform/releases/tag/v0.5.0) 以 Release 记录为准。
实施、数据备份及验证边界见[本轮记录](docs/expert-knowledge-iteration-20260909.md)和[验证记录](VALIDATION.md)。

| 能力 | 状态 | 已验证范围与限制 |
| --- | --- | --- |
| 专家与三角色导航 | `AVAILABLE` | 普通用户三栏，ADMIN/EXPERT 独立知识库管理；专家管理知识、共享 Chat 和索引，查看审计；全部用户管理及全局 Embedding/Reranker/GGUF 配置仅 ADMIN；晋升后原识别码、票据和新分机源码兼容 |
| 个人与共享诊断模型 | `AVAILABLE` | 普通用户私人 API，管理角色默认共享也可私人；个人选择优先，无选择跟随共享默认；诊断、问答、提炼、审核和整理/归类固定当前模型快照；失效明确停止，不静默换模型；API Key 隔离 |
| 外部模型端点 | `AVAILABLE` | 接受有效 HTTP(S) Base URL，不再要求 MODEL_ENDPOINT_ALLOWLIST 或私网启用开关；仍校验协议、URL 语法及受管 sidecar 身份；新流程授权默认开启，历史关闭保留 |
| 普通知识与必读 Skill | `AVAILABLE` | ENGINEER 管理自己的普通草稿、上传 Wiki/案例、AI 提炼，对共享知识或 Skill 提议修改/删除；专家/管理员审核后发布。只有管理角色直接管理 Skill；私有草稿不进入共享检索或必读方法 |
| 审批与重启恢复 | `AVAILABLE` | 原稿、逐轮 AI 修正、差异和精确审批持久化；案例提交同事务进入统一队列；发布任务与审批同事务保存，文档/向量/图谱一起切换；中断恢复保留审批，旧有效代持续可读；终态失败可重试同审批 |
| 类别与完整 Skill 阅读 | `LIMITED` | 类别新增/改名/停用同步案例选项；专属及通用 Skill，缺少两者时显著标记证据诊断；Chat 分段接收完整正文并保存模型绑定回执，不静默截断。真实第三方 API 文件夹整理通过，但综合诊断分别被上游服务器错误和 Skill 阅读格式校验阻断；CLI terra 合成完整诊断通过，真实业务质量仍未验收 |
| 指定六文件组网包 | `AVAILABLE` | 0.5.1 服务器安装包内含指定原 ZIP、六文件目录及空库初始化入口，包内独立启动验证六份 Skill、依赖和报告格式生效；已有知识、修改和删除记录不被覆盖。早期本机开发库导入仍保留；业务正文仅作为授权安装载荷，不加入源码库 |

前端生产构建、26 项新增浏览器场景、定向后端检查和实际本机页面/重启检查通过。
进程中断恢复已用合成资料验证；不等同于真实断电、公司模型、远端 PostgreSQL/Qdrant 或新版安装包验收。

### 三栏工作台改版（0.4.0，历史发行基线）

0.4.0 增量以[实施与接续记录](docs/workbench-iteration-20260908.md)及各模块交接为准；[完整服务器EXE与分机ZIP](deploy/releases/20260909/0.4.0/README.md)已构建并通过完整性和隔离包内运行检查。
当时规则替代了更早的个人修订权限：普通用户只读已发布知识、提交案例与报告，管理员确认共享；知识修改/发布仅管理员。
已实现三栏导航、问题类别与知识用途、完整Skill文件夹对话整理和单次确认原子发布、固定知识/模板快照、四章多格式报告。
新案例及知识助手默认开启模型出站授权，历史显式关闭不变；仍执行已配置端点策略。
后文M0–M5中的个人修订/原发布者审批是0.3.x历史交付范围，不表示0.4.0继续向工程师开放写权限。

| 0.4.0 增量 | 状态 | 已验证范围与限制 |
| --- | --- | --- |
| 故障定位工作台与角色界面 | `AVAILABLE` | 三栏导航；类别与个人预设模型；事件/时间线独立页面移除，保留报告设备轨迹与问答；生产构建及9项模拟浏览器检查通过 |
| 分类与固定知识上下文 | `AVAILABLE` | 对应类别加通用知识，未知跨类查找并给有证据的类别建议；固定内容/分块/模板，保留完整Skill依赖与跨类说明；历史快照不随发布变化 |
| 管理员知识助手 | `LIMITED` | 服务器Chat API完成全文分段阅读、查询、纠偏和精确差异清单；最多64个UTF-8文件，单个1MiB/合计8MiB，不静默截断；会话30轮/32000字符；56项合成检查通过，真实公司模型语义质量待验收 |
| 单次确认与原子发布 | `AVAILABLE` | 人工确认固定变更摘要后生成持久任务；文档、向量、图谱同时切换；取消/失败/并发修改保留旧代，接管任务受租约保护；失败私有代保留，尚无新增垃圾回收 |
| 已定位案例与已确认报告 | `AVAILABLE` | 负责人提交完成的关联诊断，服务端固定报告；管理员带版本确认后共享，待审材料不用于其他案例检索；普通用户无知识写入权 |
| 类别报告模板及安全导出 | `LIMITED` | 默认使用用户原始规范整理的组网v2模板；四章HTML/Word/PDF共用正文，证据白名单和置信度/处理优先级分开；41项新检查和PDF视觉核验通过，Word实际分页未验证 |

规范型自定义Markdown模板的自然语言要求传给模型；结构和引用校验不能替代语义审核。
本轮不重跑历史回归、Full、External或手动CI；未完成新CI和公司实机验收，不宣称已满足合并/正式发布门禁。

- 0.3.3 Embedding 热修复：内置 BGE 按实际 tokenizer 计数（含特殊 token）将超长输入完整分窗，每窗不超过 512 tokens；所有窗口向量按 token 数加权并归一化。原文、分块 ID 和历史引用保持不变。541/2252-token 合成输入已用随包真实模型验证；长文检索质量尚未单独评测。安装热修复后需在设置页重建向量索引。
...
- 0.3.3 内网专用包增加免密码登录：个人识别码严格为一位小写字母加八位 ASCII 数字，自助创建/接续 ENGINEER，拒绝管理员及停用账号。
  同一码保持同一用户 ID；身份为用户自报，不校验姓名或密码。分机安装时自动配置当前用户证书并保存个人凭据。
  桌面网页入口通过一分钟内有效、仅能兑换一次的凭据接续 CLI 身份；直接访问网页可输入同一码并记住登录。
  地址只进入专用构建产物；通用部署默认不开启该模式，知识修订与发布审批继续使用原用户 ID 和角色规则。
