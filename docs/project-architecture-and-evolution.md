...
2026-09-09 迭代中，普通用户可增删改、测试和使用自己的私有 Chat Profile；管理员/专家默认创建共享配置，也可选择私有。个人选择优先，未选择时跟随共享默认；共享默认只能指向共享模型，任何角色都不能使用他人的私有配置。全局 Embedding/Reranker/GGUF 只允许管理员配置，专家可重建知识索引。Embedding 切换后必须重建知识向量；旧向量按 `profile_id` 隔离，不会误用到新模型。

API Key 和 Chat 模型代理 URL 在后端使用 Fernet 加密，不回显明文，脱敏提示也仅向配置所有者展示。外部 API 和代理在保存、激活和实际请求前验证 HTTP(S) 协议与 URL 格式，不按主机/IP 范围拦截；生产环境、私网与回环地址均无需端点白名单或私网放行开关。托管 GGUF sidecar 继续校验运行时身份和凭据。代理为空时 Profile 直连；启用代理时使用独立 HTTPX Client，保留证书链和主机名验证并清除 CRL 吊销检查标志。
...
| 2026-08-11 | 历史修改 | 私网端点与通用知识范围 | 当时使用一次性脚本放行私网模型，2026-09-09 迭代已取消该要求；知识新增 GENERAL 通用适用范围并兼容旧 OTHER 检索 |
