...
[0.5.2 GitHub Release](https://github.com/mrrabbitss/debug-platform/releases/tag/v0.5.2) 已发布为最新版，
服务器 EXE、相同的 0.5.0 分机 ZIP、服务器/分机说明、更新说明、交付清单及 SHA-256 共七份附件已远端核验。
源码标签对应 `fd5336516c0930020537b84ebdfd36bc39bc11be`；服务器包继续包含六份原始组网 Skill 和管理员恢复工具。
升级停止服务器并备份后覆盖安装，无需卸载；已有知识不自动清空。详细步骤见 [服务器指南](服务器使用指南.md)。
