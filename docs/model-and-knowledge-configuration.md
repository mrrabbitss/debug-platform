...
以下模型权限与地址规则对应 2026-09-09 迭代源码；已安装的旧发行包需要升级到包含本次修改的版本后才适用。
...
- `model_profile_access`：模型配置的创建者及 `SHARED`/`PRIVATE` 可见性；个人模型选择单独保存在用户偏好中。
...
和跨模块候选。调用遵循用户的模型出站授权；新案例和知识助手默认开启出站，历史明确
关闭的记录不会自动改写。配置地址时无需另外申请平台端点白名单。

后端会在保存、启用和每次实际调用前验证 Base URL：

- 仅支持 `http://` 和 `https://`，不允许 URL 内嵌账号密码、查询串或片段；
- 内网域名、私网 IP、回环地址及公网模型地址均可直接使用，不按主机名或 IP 范围拦截；
- 开发与 `APP_ENV=prod` 使用相同规则，HTTP 地址也不要求额外放行；
- 平台管理的 GGUF sidecar 仍校验运行时身份、回环端口和凭据，不能用任意地址冒充托管模型。

Chat 模型代理同样没有主机白名单或私网开关要求。代理 URL 仅支持 `http://` 或
`https://`，可以包含认证信息，但不能包含路径、查询串或片段；返回给前端的提示会移除
用户名和密码，且只向配置所有者显示该提示。HTTPS 的证书链和主机名校验继续生效。

旧 `.env` 中的 `MODEL_ENDPOINT_ALLOWLIST` 和 `MODEL_ALLOW_PRIVATE_ENDPOINTS` 仅为升级
兼容保留，不再参与 API/代理地址判定。无需添加这些变量，也无需运行旧私网放行脚本。
如果升级后仍出现 `production model endpoints must be listed`，应确认运行的是已更新的
后端进程或安装包，而不是继续补白名单。
...
管理员和专家在左侧“知识库管理”维护 Skill、分类和共享知识，普通用户在“知识库”查看
已发布 Skill、上传案例/Wiki Markdown、修改自己的草稿并使用 AI 案例提炼。对共享内容
提出的修改、删除和 Skill 修订建议须由管理员或专家审核；普通上传不直接生成必读 Skill。
...
网页端由管理员或专家在“知识库管理”使用“AI 智能导入 MD”，选择一个或多个文件、自己可用的 Chat Profile、
...
两条路径都要求管理员或专家权限，并且每个输入文件最终只得到一个 `active=false`、
`review_status=DRAFT` 的文档。分类本身不会提交审核或发布；管理员或专家仍需检查标题、分类、设备、
...
普通用户（ENGINEER）可以新增、修改、测试、删除自己的私有 Chat API。管理员和专家新建
Chat 配置时默认全员可用，也可选“仅自己使用”。其他人的私有模型不会出现在列表中，
管理员也不能通过配置 ID 查看或使用它。API Key 始终不回显。

历史 VIEWER 账号保留只读权限，可选择和测试自己可见的模型，不因此获得配置增删改权限。
全局 Embedding、Reranker、GGUF 及用户管理只有管理员可修改；专家可以使用这些模型并重建知识索引。
...
保存后先点击“测试”，再在系统设置选择个人使用的诊断模型。个人选择统一用于新发起的
诊断、问答和 AI 提炼；未选择时跟随共享默认。管理员或专家可设置共享默认，但只能选择
共享 Chat 配置。个人选择失效时会明确报错，不会悄悄改用其他模型。历史 Chat 配置升级为
共享，已有个人偏好保留；`.env` 中 `LLM_*` 配置的首次导入兼容路径也保留。
...
和其中的账号密码使用与 API Key 相同的 Fernet 密钥加密，仅配置所有者能看到不含凭据的
...
2. 添加并测试自己的诊断大模型 API，或选择管理员/专家提供的共享模型；
