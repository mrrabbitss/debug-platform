...
执行 TXT/HTML/DOCX/PDF 文件夹上传、三类文档预览、初稿生成、对话纠错、人工确认只创建
`DRAFT`、运行轨迹查看与脱敏重放检查。页面异常、请求失败、未捕获异常或浏览器控制台错误
都会使测试失败。
...
当前线上 Agentic Search 仍以确定性 Planner 为默认安全基线；有界循环执行器已经具备测试和
Golden 门禁，但尚未默认替换检索器。后续若接入 LLM Planner，必须继续复用相同 Tool
Registry、预算、轨迹、审批和回退边界。
