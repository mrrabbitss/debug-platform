...
需要生成一个可直接在前端讲解的 AP 频繁离线案例时，可运行：

```bat
scripts\seed_ap_offline_demo.bat --approve-model-egress
```

该命令只上传仓库内的纯合成 GW/AP 日志，且要求显式同意模型出站；`--local-only` 可完全跳过
外部模型。种子器会校验两侧非空日志规划、方法完整阅读、1 至 20 轮模型规划、模型实际发起
有命中的 `search_log`，以及后端按前置搜索 ID 执行并明确标注的 `get_evidence` 证据水合；
策略水合不冒充模型调用。它还校验 27 个故障树节点、四类跨设备假设、物理链路排除、UDM 根因支持、
网络传输证据不足和本地 UDN/MAC 等值证据；报告还必须包含两侧文件行号且不得出现内部证据
主键。每个后台任务默认最多等待 1,200 秒，可用 `--job-timeout-seconds` 在 30 至 14,400 秒间
调整，避免演示脚本无限等待。详见 [AP 频繁离线演示](demo-ap-frequent-offline.md)。
...
`search_knowledge`、`search_log` 或 `get_evidence`，重复调用复用前次结果。每轮最多四个模型规划调用；
首个非空模型日志搜索可以触发一次独立策略证据读取，该读取计入总工具预算但不挤占模型调用槽位。
工具名、Pydantic 输入输出、
角色、只读权限、方法/Pattern/节点/evidence ID 都在执行前经过校验；尚未绑定检查和证据工具的节点
不得提前进入终态，单轮输出最多纠正两次；失败后显示稳定诊断并保留确定性结果。
该生产循环还累计所有规划轮次的输入/输出/总 Token、墙钟时间和实际只读工具调用；每轮请求前清空
供应商观测快照，因此模型调用前失败不会重复计入上一轮 Token。连续多轮既没有
