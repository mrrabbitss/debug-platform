...
逐条核对、与模型多轮纠错并人工确认，再提交知识提案。管理员或专家可继续修正并审核；
审核批准同时构成发布审批，索引构建成功后才对全员生效。以下权限和模型规则对应
2026-09-09 迭代源码，旧发行包需升级后适用。

## 1. 使用前准备

1. 运行 `scripts\start_local.bat`。
2. 进入“系统设置 → 诊断大模型”，新增自己的 OpenAI-Compatible Chat API，或选择
   管理员/专家提供的共享模型。普通用户的配置仅本人可见；管理员/专家默认共享，也可设为私有。
3. 测试后保存个人选择；未选择时跟随共享默认。内网、localhost 和公网的有效 HTTP(S)
   Base URL 均可直接配置，生产环境也无需端点白名单或私网放行脚本。
...
1. 在左侧“知识库”打开“AI 案例提炼”。普通用户、专家和管理员都可以使用自己的提炼会话。
2. 点击“选择文件夹并提炼”，选择整个案例文件夹。
3. 核对个人默认或本次选择的可见 Chat 模型、知识分类、可信级别和保密级别；设备、型号、
   固件和模块可预填。会话固定创建时选择的模型，之后修改个人默认不会切换已有会话。
4. 核对“模型数据出站”；新会话默认开启，明确关闭时不会向 API 模型发送证据，历史关闭记录保留。
...
8. 当结构和引用校验通过后，确认生成知识草稿与自己的未发布提案。
9. 在“知识库”的提案中提交审核。管理员或专家在左侧“知识库管理”的审核队列核对内容，
   可人工修改或用 AI 多轮修正，再批准最终版本。原稿、修改差异和审核记录保留；索引发布
   成功后才进入共享检索，普通用户不能自行发布或直接改动必读 Skill。
...
浏览器文件夹
  ...
  → 用户已授权出站的会话固定 Chat 模型 API
  → 结构化 JSON + Markdown 草稿
  → 后端章节/来源/行号校验
  → 人机多轮校正与不可变版本
  → 人工确认
  → 不可检索的 KnowledgeDocument DRAFT 与本人知识提案
  → 本人提交，管理员/专家审核最终版本
  → 索引发布成功后参与共享检索
```
...
提示注入的风险；规则脱敏不等同于完整 DLP。模型接收的数据由会话出站授权决定，平台
不使用 `MODEL_ENDPOINT_ALLOWLIST` 或 `MODEL_ALLOW_PRIVATE_ENDPOINTS` 限制外部 API 地址。
HTTP(S) 地址格式、TLS 校验和托管 GGUF 身份校验仍保留。
...
用于知识 provenance。用于权限校验的模型绑定由配置 ID、发起用户和配置指纹组成；
API Key 和代理凭据不进入会话快照，审核他人提案时不返回其私有模型信息。后台调用会重新
校验模型权限和指纹；配置、凭据或可见性变化时明确失败，不会切换到全局模型。
审计日志不记录正文和提示词。
...
`CONFIRMED` 只表示人工确认并创建了知识草稿与提案，不等于知识已经发布。

## 6. 常见问题

### 页面没有可选模型

内置 Mock 模型不能用于文件夹提炼。请在系统设置中创建自己的 Chat 类型
OpenAI-Compatible API Profile，或选择可见的共享 API 模型。本项目当前尚未提供本地 Chat 推理运行器；本地 BGE 和
...
检查模型 Profile 的连接测试、Base URL 格式与网络连通性、API Key、超时和模型 JSON 输出能力。
平台没有端点白名单要求。若提示会话模型配置已变化或权限已失效，应重新明确选择模型并发起
请求；后台不会自动使用其他人的私有模型或替换为共享默认。
...
本人提炼会话接口前缀为 `/api/v1/knowledge-curations`，允许 ADMIN、EXPERT、ENGINEER：
...
- `POST /{id}/confirm`：人工确认并创建知识库 `DRAFT` 与本人知识提案；
- `POST /{id}/retry`、`DELETE /{id}`：重试或删除未入库会话。

交互式请求结构可以在启动后的 `/docs` 查看。会话读写检查创建者；管理员/专家只有在来源
已用于提交的知识提案时才获得审核所需的只读访问，不能直接改写他人的私有提炼会话。
审核修改在 `/api/v1/knowledge-contributions/{id}/review-draft`、`review-chat` 和 `review`
接口完成，仅管理员/专家可执行。普通用户通过本人的提案提交共享知识改动或 Skill 修订建议，
没有直接管理或发布 Skill 的权限。历史 VIEWER 仍只有只读访问与个人模型选择权限。
