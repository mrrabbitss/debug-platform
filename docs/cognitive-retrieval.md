...
4. 后端以 Pydantic 和证据 ID 验证模型输出；兼容 GLM 常见的结构名根对象、字符串关键词和旧 `plan` 字段，校验失败时把错误与必读 ID 回送模型纠正一次，两次仍失败或模型不可用才回退确定性规划；
...
GW/AP 联合方法、三层日志证据、以前轮次和检索观察，并要求逐方法输出相关性判断，每个检查和检索
query 都引用有效方法 ID。案例症状与故障树文本直接重合时，首轮不得把该故障树判为无关，也不得缺少
对应检查和实际检索。综合 Planner 同样接受 GLM 常见富对象字段；Schema、必读 ID、相关性或方法绑定
校验失败时会携带错误和必需 ID 纠正一次，并累计两次调用 Token。Planner 每轮最多产生四个去重检索
query；证据足够、达到轮次上限、校验失败、取消或预算耗尽都会产生明确停止原因。案例页可展开查看
每轮“方法相关性 → 检查 → 实际调度查询”，轨迹同时显示结构纠错次数和查询来源方法。
