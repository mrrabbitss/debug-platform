...
- [x] GW/AP 联合日志规划、两至八轮综合诊断、GLM 富对象归一化/纠错重试、方法相关性与检查/检索绑定门禁、异步案例问答和人工确认诊断修订的持久化任务与案例级实时轨迹；
...
- [x] Chat JSON 模式、供应商 usage 映射、校验失败用量保留和前端阶段用量回算回归；
...
- [x] 综合诊断 Planner 将 GLM 输出 Schema、归一化和方法覆盖门禁拆为独立契约模块，降低多轮执行器的上下文和改动冲突；
