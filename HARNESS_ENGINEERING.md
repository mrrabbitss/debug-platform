...
首次构建入口 `setup_server.bat` 统一调用四个既有构建步骤，选择 Python 3.12 并验证 Node/Git。
构建互斥、逐步失败即停，完整性校验后才发布最终目录；已有程序校验后复用，不覆盖业务数据。
PowerShell 回归覆盖步骤失败、末尾校验失败、目录发布及已有目录保护。
2026-09-08 修正一键构建漏传镜像参数：模型准备固定使用 `hf-mirror.com`；回归检查实际传给准备脚本的参数。
