...
0.5.6 实际服务器 EXE 验收通过：移除服务器安装中的固定 app 目录切换和临时目录组件裁剪；
通过 PrepareToInstall 传播发布失败，完整校验及六文件更新通过后才注册新启动入口。
实际验证占用时退出 7、同时持有新旧目录句柄时成功升级、首次启动完整六文件入库及管理员创建。
已通过的新场景复用同一 EXE 证据，未重复旧成功回归。旧通用便携安装器行为不变。
审计见 [两版安装变更](docs/installer-056-change-audit.md)，确切包哈希及范围见 VALIDATION.md。
0.5.6 已设为最新正式版，八份远端附件及源码标签验证一致；自动重复打包任务
34441809888、34441810030 已取消，无手动 CI 或 CI 全绿声明。

新增只更新六文件组网 Skill 的停服工具，使用独立作业类型隔离旧版本全库重置处理器。
仅定向验证替换范围、备份/重试、旧包兼容及双击入口，不重跑既有成功套件或调用 Chat/CLI。
源文件只进入忽略的交付附件，不提交 Git；实现边界见
[独立更新工具](docs/offline-network-skill-update-20260910.md) 和 VALIDATION.md。
